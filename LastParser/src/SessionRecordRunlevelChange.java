 ...
 */
public class SessionRecordRunlevelChange extends SessionRecord {

    public Object parseRecord(String inputString){

        Pattern patternSignature = Pattern.compile("runlevel \\(to lvl \\d\\)"); //	system crash characteristics
        Matcher matcherSignature = patternSignature.matcher(inputString);

        Pattern patternOverall = Pattern.compile("runlevel \\(to lvl \\d\\)" + "\\s+" +
                dataTimePatternString + "\\s-\\s" +dataTimePatternString +
                "\\s+.+");
        ...
        //System.out.println(matcherDateTime.find());
        if (matcherSignature.find()){
            if (matcherOverall.find()){
                SessionRecordRunlevelChange sr = new SessionRecordRunlevelChange();
