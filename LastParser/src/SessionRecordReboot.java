 ...
 */
public class SessionRecordReboot extends SessionRecord{

    public Object parseRecord(String inputString){


        Pattern patternSignature = Pattern.compile("reboot   system boot"); //	system crash characteristics
        Matcher matcherSignature = patternSignature.matcher(inputString);

        Pattern patternOverall = Pattern.compile("reboot   system boot" + "\\s+" +
        ...
        if (matcherSignature.find()){
            if (matcherOverall.find()){
                SessionRecordReboot sr = new SessionRecordReboot();
